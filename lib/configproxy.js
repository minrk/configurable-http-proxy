...
class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    if (this.options.apiSsl) {
      this.apiServer = http2.createSecureServer({allowHTTP1: true, ...this.options.apiSsl}, apiCallback);
    } else {
    ...
    if (this.options.ssl) {
      this.proxyServer = http2.createSecureServer({...serverOptions, ...this.options.ssl}, proxyCallback);
    } else {
      this.proxyServer = http.createServer(serverOptions, proxyCallback);
    }
    // proxy websockets
  ...
  handleProxy(kind, req, res) {
    // proxy any request
    var that = this;
    // console.log("handleProxy", kind, req, res)
    ...
    return this.targetForReq(req)
      .then((match) => {
        ...
        args.push(function (e) {
          ...
          that.metrics.requestsProxyCount.labels(res.statusCode).inc();
